...
export function ApplyExpert({ onBack }: ApplyExpertProps) {
    ...
    const loadState = async () => {
      ...
      try {
        const [meResponse, statusResponse] = await Promise.all([
          ...
          api.get<StatusResponse>('/expert-applications/my-status'),
        ]);
  ...
  const handleApply = async () => {
    ...
    try {
      ...
      const response = await api.post('/expert-applications');
    ...
    <div className="bg-[#121212]/80 backdrop-blur-xl border border-[#A5C89E]/30 rounded-2xl p-12 text-center">
      <div className="mb-6 flex justify-center">
        <div className="p-6 bg-[#A5C89E]/10 border border-[#A5C89E]/30 rounded-2xl">
          {isBlocked ? <CheckCircle2 className="w-12 h-12 text-[#A5C89E]" /> : <FileText className="w-12 h-12 text-[#A5C89E]" />}
        </div>
      </div>

      {!isBlocked ? (
        <>
          <h2 className="text-2xl font-bold text-white/90 mb-3">You are not an expert writer yet</h2>
          ...
          >
            ...
            <span>{isSubmitting ? 'Submitting...' : 'Apply to Become an Expert Writer'}</span>
          </button>
        ...
        <>
          ...
          <p className="text-gray-400 mb-6 leading-relaxed max-w-md mx-auto">
            Your expert writer application is currently {applicationStatus}. You cannot submit a duplicate application now.
          </p>
